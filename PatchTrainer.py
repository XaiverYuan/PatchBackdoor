...
def readData(report: Dict) -> None:
    """
    given a report generated by train, visualize all the data
    ...
    transL = report['transCount']
    trainCount = report['train count']
    valiCount = report['vali count']
    # load them as tensor, so it could be operated easily
    trainLoss = torch.Tensor(report['train loss'])
    trainAcc = torch.Tensor(report['train acc'])
    valiAcc = torch.Tensor(report['vali acc'])
    # dummy variable for drawing
    x = [round + 1 for round in range(ROUNDS)]
    # make the loss average loss and calculate the sum loss
    trainLoss /= trainCount
    trainSumLoss = torch.sum(trainLoss, dim=1)
    # create the figure
    fig = plt.figure(figsize=(10, 10))
    colors = ['b', 'r', 'g', 'y', 'c']

    # train loss plot
    axLossTrain = fig.add_subplot(2, 2, 1)
    for transI in range(transL):
        axLossTrain.plot(x, trainLoss[:, transI].data.numpy().tolist(), color=colors[transI], alpha=0.5)
    del transI, trainLoss
    axLossTrain.plot(x, trainSumLoss.data.numpy().tolist(), color='black', linewidth=3)
    axLossTrain.set_xlabel('epoch')
    axLossTrain.set_ylabel('loss')
    lossTrainLegend = [str(transI + 1) for transI in range(transL)]
    lossTrainLegend.append("Total")
    axLossTrain.legend(lossTrainLegend)
    axLossTrain.set_title('Training loss')
    del axLossTrain, lossTrainLegend, trainSumLoss

    # Draw the graph for Train Accuracy
    axAccTrain = fig.add_subplot(2, 2, 3)
    for transI in range(transL):
        axAccTrain.plot(x, trainAcc[:, transI].data.numpy().tolist(), color=colors[transI], alpha=0.5)
    axAccTrain.set_xlabel('epoch')
    axAccTrain.set_ylabel('accuracy')
    axAccTrain.set_ylim([0, 1])
    accTrainLegend = [str(transI + 1) for transI in range(transL)]
    accTrainLegend.append("Total")
    axAccTrain.legend(accTrainLegend)
    axAccTrain.set_title('Training Accuracy')
    # Draw Vali Accuracy Graph
    axAccVali = fig.add_subplot(2, 2, 4)
    for transI in range(transL):
        axAccVali.plot(x, valiAcc[:, transI].data.numpy().tolist(), color=colors[transI], alpha=0.5)
    axAccVali.set_xlabel('epoch')
    axAccVali.set_ylabel('accuracy')
    axAccVali.set_ylim([0, 1])
    accValiLegend = [str(transI + 1) for transI in range(transL)]
    accValiLegend.append("Total")
    axAccVali.legend(accValiLegend)
    axAccVali.set_title('Validation Accuracy')
    plt.show()
